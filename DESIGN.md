...
- **M1 walking skeleton**: 最小 SSA + 最ナイーブ isel + 最ナイーブ rega +
  arm64 emit で、**分岐もループも無い1関数**を as/ld まで通し実行。
  golden 1本(例: `collatz` 縮小版)が通る。
- **M2 SSA を本物に**: Braun SSA + cfg/dom。`-dN` と diff。phi/jnz/loop 対応。
