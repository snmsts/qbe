...
(defun print-mem (m stream)
  "printref RMem: `[offset + base + scale * index]` (parse.c printref)."
  (write-char #\[ stream)
  (let ((sep nil))
    (let ((off (mem-offset m)))
      (when (and off (not (eq (con-kind off) :undef)))
        (print-con off stream) (setf sep t)))
    (when (mem-base m)
      (when sep (write-string " + " stream))
      (print-ref (mem-base m) stream) (setf sep t))
    (when (mem-index m)
      (when sep (write-string " + " stream))
      (format stream "~d * " (mem-scale m))
      (print-ref (mem-index m) stream)))
  (write-char #\] stream))
