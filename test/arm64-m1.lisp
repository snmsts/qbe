;;;; arm64-m1.lisp --- G1 walking-skeleton end-to-end tests (arm64 / Apple).
;;;; The arm64 twin of m1.lisp: compiles tiny IL programs to a native executable
;;;; via `cc` on Apple Silicon, runs them, and checks the exit code.  Proves the
;;;; naive arm64 code generator (src/arm64.lisp) and the target seam end to end.
;;;; Needs a host arm64 C toolchain (cc); no qbe oracle.
;;;; usage: ros -Q run -- --script test/arm64-m1.lisp
(require :asdf)
...
(defpackage #:qbe-arm64-m1 (:use #:cl))
(in-package #:qbe-arm64-m1)
...
(defun check (name src expect)
  ...
  (handler-case
      (multiple-value-bind (code out asm) (qbe:a64-compile-and-run src)
        (declare (ignore out))
        (if (eql code expect)
...
(defun check-drv (name src cdrv expect)
  ...
  (handler-case
      (let ((asm (qbe:a64-module-asm-string (qbe:parse-string src))))
...
(format t "~&G1 end-to-end (arm64 / Apple):~%")
...
;; loaduw into an l temp must zero-extend.  Store 0xffffffff (-1 as w), load
...
(format t "~&=== G1/arm64: ~d passed, ~d failed ===~%" *pass* *fail*)
