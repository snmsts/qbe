;;;; abi.lisp --- M3/B1 oracle: our amd64 SysV abi1 vs `qbe -dA`'s
;;;; "> After ABI lowering:" printfn dump.
;;;;
...
(defun da-dump (ssa-path)
  (let ((g (merge-pathnames
            ...
            (merge-pathnames "test/golden-da/"
                             (asdf:system-relative-pathname "qbe-cl" "")))))
    (if (probe-file g)
        (uiop:read-file-string g)
        (multiple-value-bind (out err)
            (uiop:run-program (list *qbe-path* "-dA" "-o" *null-device* (namestring ssa-path))
