;;;; gvn.lisp --- M2/C3 oracle: our gvn vs `qbe -dG`'s "> After GVN:" dump.
;;;;
  ...
  (let ((g (merge-pathnames
    ...
    (if (probe-file g)
        ...
        (multiple-value-bind (out err)
            (uiop:run-program (list *qbe-path* "-dG" "-o" *null-device* (namestring ssa-path))
                              :output :string :error-output :string :ignore-error-status t)
          (declare (ignore out)) err))))

(defun dg-sections (dump)
  "Alist fn-name -> the '> After GVN:' body."
    ...
    (labels ((flush ()
      ...
      (dolist (line (uiop:split-string dump :separator '(#\Newline)))
        (cond
          ...
          ((string= line "> After GVN:") (setf acc '() in t))
...
(let ((raw 0) (norm 0) (tot 0))
  ...
  (format t "~&=== gvn / After GVN (normins-only slice) ===~%")
