...
@start
	...
	%in.20 =l add %in.17, 12
	%t3.102 =w loadsw %in.20
	%in.21 =l add %in.17, 16
	%t4.103 =w loadsw %in.21
	%in.22 =l add %in.17, 20
	%t5.104 =w loadsw %in.22
	%in.23 =l add %in.17, 24
	%t6.105 =w loadsw %in.23
	%in.24 =l add %in.17, 28
	%t7.106 =w loadsw %in.24
	%in.25 =l add %in.17, 32
	%t8.107 =w loadsw %in.25
	%in.26 =l add %in.17, 36
	%t9.108 =w loadsw %in.26
	%in.27 =l add %in.17, 40
	%t10.109 =w loadsw %in.27
	%in.28 =l add %in.17, 44
	%t11.110 =w loadsw %in.28
	%in.29 =l add %in.17, 48
	%t12.111 =w loadsw %in.29
	%in.30 =l add %in.17, 52
	%t13.112 =w loadsw %in.30
	%in.31 =l add %in.17, 56
	%t14.113 =w loadsw %in.31
	%in.32 =l add %in.17, 60
	%t15.114 =w loadsw %in.32
	%isel.230 =w copy 10
@loop
	%counter.116 =w phi @start %isel.230, @loop %counter.229
	...
	%isel.231 =w copy 10
	%counter.229 =w sub %counter.116, %isel.231
	jnz %counter.229, @loop, @done
@done
	storew %t0.187, %out.98
	%out.1 =l add %out.98, 4
	storew %t1.199, %out.1
	%out.2 =l add %out.98, 8
	storew %t2.211, %out.2
	%out.3 =l add %out.98, 12
	storew %t3.223, %out.3
	%out.4 =l add %out.98, 16
	storew %t4.228, %out.4
	%out.5 =l add %out.98, 20
	storew %t5.192, %out.5
	%out.6 =l add %out.98, 24
	storew %t6.204, %out.6
	%out.7 =l add %out.98, 28
	storew %t7.216, %out.7
	%out.8 =l add %out.98, 32
	storew %t8.214, %out.8
	%out.9 =l add %out.98, 36
	storew %t9.226, %out.9
	%out.10 =l add %out.98, 40
	storew %t10.190, %out.10
	%out.11 =l add %out.98, 44
	storew %t11.202, %out.11
	%out.12 =l add %out.98, 48
	storew %t12.201, %out.12
	%out.13 =l add %out.98, 52
	storew %t13.213, %out.13
	%out.14 =l add %out.98, 56
	storew %t14.225, %out.14
	%out.15 =l add %out.98, 60
